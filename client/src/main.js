...
async function ensureStudentCanEnterClassroom(roomCode) {
  const joinResult = await api(
    `/classrooms/${encodeURIComponent(roomCode)}/join`,
    {
      method: "POST",
    },
  );
...
function sendStudentBackToDashboard(
  role,
  message = CLASS_ENDED_NOTICE,
  showModal = false,
) {
  ...
  return await new Promise((resolve, reject) => {
    ...
    const handleRoomDeleted = (message) => {
      ...
      sendStudentBackToDashboard(
        ...
        typeof message === "string" && /deleted|ended/i.test(message)
          ? CLASS_ENDED_NOTICE
          : "This classroom has been deleted.",
    ...
    const onRoomError = (payload = {}) => {
      ...
      if (
        /session is not started|wait for the teacher|teacher has not joined/i.test(
          message,
        )
      ) {
        try {
        ...
        sendStudentBackToDashboard(
          ...
          message ||
            "Class session is not started yet. Please wait for the teacher to enter the classroom.",
...
async function handleExitFromClassroom(
  { role, roomCode, isTeacher },
  exitAction,
) {
  if (!isTeacher) {
  ...
  if (exitAction?.action === "end_call") {
    ...
    try {
      await api(`/classrooms/${encodeURIComponent(roomCode)}`, {
        method: "DELETE",
      });
    } catch (error) {
  ...
  return sharedRooms
    ...
    .map((room) => {
      ...
      return {
        ...
        participantNames: Array.isArray(room.participantNames)
...
async function mountLegacyClassroom(params) {
  ...
  page.loadButton.addEventListener("mouseenter", warmupClassroomModules, {
    once: true,
  });

  page.loadButton.addEventListener("touchstart", warmupClassroomModules, {
    once: true,
  });

  page.loadButton.addEventListener("focus", warmupClassroomModules, {
    once: true,
  });

  if (classroomLoader.lowBandwidth) {
    page.setStatus(
      "Low bandwidth detected. 3D is paused to keep the app responsive.",
      "Low bandwidth",
    );
  } else {
...
async function renderRoute() {
  ...
  const isClassroomRoute =
    path === "/classroom" || path === "/room" || path === "/group-discussion";
  ...
  if (path === "/classroom") {
    ...
    if (roomCode) {
      if (roomRole === "student") {
        try {
          ...
          localStorage.setItem(
            ...
            error?.message || "Could not enter classroom.",
          );
      ...
      const session = createRuntimeSession(
        new URLSearchParams(`role=${roomRole}&roomCode=${roomCode}`),
      );
      ...
      const checkAndInitVoice = async () => {
        if (window.activeClassroomSocket && !activeVoiceSystem) {
          try {
            ...
            activeVoiceSystem = new VoiceSystem(
              window.activeClassroomSocket,
              window.activeClassroomSocket.id,
              roomRole,
            );
            ...
            if (page.muteButton) {
              ...
              window.activeClassroomSocket.on(
                "audio-state-change",
                ({ userId, muted, by }) => {
                  ...
                  if (by && !isMuted) {
                    page.setStatus(
                      "Teacher unmuted your microphone.",
                      "Mic unmuted",
                      true,
                    );
            ...
            if (page.deafenButton && roomRole === "student") {
              page.deafenButton.addEventListener("click", () => {
                ...
                page.deafenButton.setAttribute(
                  "data-tooltip",
                  isDeafened ? "Unmute Teacher" : "Mute Teacher",
                );

                page.deafenButton.style.color = isDeafened
                  ? "#dc2626"
                  : "#64748b";
              });
            }
        ...
        try {
          ...
          const renderRaiseHands = (list) => {
            ...
            (list || []).forEach((entry) => {
              ...
              const displayName =
                entry && entry.displayName ? entry.displayName : userId;
              ...
              unmuteBtn.addEventListener("click", () => {
                ...
                socket.emit("teacher-set-audio-state", {
                  target: userId,
                  muted: nextMuted,
                });
                ...
                try {
                  if (
                    activeVoiceSystem &&
                    typeof activeVoiceSystem.enableRemoteAudioWithGesture ===
                      "function"
                  ) {
                    activeVoiceSystem.enableRemoteAudioWithGesture();
                  }
          ...
          socket.on("raise-hand-list", (list) => {
            try {
              renderRaiseHands(list);
            } catch (e) {
              console.warn("Render raise-hands failed", e);
            }
          });

          socket.on("unmute-request", ({ userId, displayName }) => {
            const existing = page.raiseHandList.querySelector(
              `[data-user-id="${userId}"]`,
            );

            if (!existing) {
              const current = page.raiseHandList
                ? Array.from(page.raiseHandList.querySelectorAll("li")).map(
                    (li) => ({
                      userId: li.dataset.userId,
                      displayName:
                        li.querySelector(".dc-raise-hand-user")?.textContent ||
                        li.dataset.userId,
                    }),
                  )
                : [];
              ...
              renderRaiseHands([
                ...current,
                { userId, displayName, muted: true },
              ]);
            }

            page.setStatus(
              `${displayName || "Student"} requested microphone access.`,
              "Mic request",
              true,
            );
          });
      ...
      page.loadButton.addEventListener(
        "mouseenter",
        () => classroomLoader.warmup().catch(() => {}),
        { once: true },
      );
  ...
  if (path === "/login") {
    ...
    mountLogin(appRoot, {
      ...
      onGoRegister: (nextRole = role) =>
        navigate(`/register?role=${nextRole}&mode=signup`),
  ...
  if (path === "/register") {
    ...
    mountRegister(appRoot, {
      ...
      onGoLogin: (nextRole = role) =>
        navigate(`/login?role=${nextRole}&mode=login`),
  ...
  if (path === "/join") {
    const { mountJoinClassroomPage } =
      await import("./features/dashboard/joinClassroomPage.js");
    ...
    mountJoinClassroomPage(appRoot, {
      ...
      onSubmit: async (code) => {
        ...
        const joinResult = await api(
          `/classrooms/${encodeURIComponent(room)}/join`,
          {
            method: "POST",
          },
        );
  ...
  if (path === "/create") {
    const { mountCreateClassroomPage } =
      await import("./features/dashboard/createClassroomPage.js");
  ...
  if (path === "/room") {
    ...
    const roomCode = String(params.get("code") || "")
      .trim()
      .toLowerCase();
    ...
    if (!getToken()) {
      ...
      navigate(
        `/login?role=${roomRole}&mode=login&next=${encodeNextPath(roomPath)}`,
      );
    ...
    try {
      const { socket } = await startSocketClassroom({
        roomCode,
        role: roomRole,
      });
  ...
  if (path === "/group-discussion") {
    ...
    const { mountRoomToolPage } =
      await import("./features/dashboard/groupDiscussionPage.js");

    if (!window.activeClassroomSocket && getToken()) {
      try {
        const { socket } = await startRoomSocketOnly({
          role: routeRole,
          roomCode,
        });
        ...
        console.warn(
          "Discussion live socket unavailable; using saved discussion API.",
          error,
        );
      }
    }
    ...
    mountRoomToolPage(appRoot, {
      ...
      onBack: () =>
        navigate(
          `/classroom?role=${routeRole}&code=${encodeURIComponent(roomCode)}`,
        ),
  ...
  if (path === "/notes") {
    ...
    const { mountNotesPage } =
      await import("./features/dashboard/notesPage.js");
  ...
  if (path === "/profile") {
    const { mountProfilePage } =
      await import("./features/profile/profilePage.js");
  ...
  const { mountDashboard } =
    await import("./features/dashboard/dashboardPage.js");
  ...
  mountDashboard(appRoot, {
    ...
    onRoomSelected: ({ roomCode, role: classroomRole }) => {
      if (classroomRole === "student") {
        ensureStudentCanEnterClassroom(roomCode)
          .then((entry) => {
            ...
            navigate(
              `/classroom?role=student&code=${encodeURIComponent(roomCode)}`,
            );
          })
      ...
      navigate(
        `/classroom?role=${classroomRole}&code=${encodeURIComponent(roomCode)}`,
      );
    },
